...
def run(context):
    last_article_on_news_page = False
    attempts = 0

    last_article_on_news_page = element_exists(context, 'div_will_bots')
    
    while not last_article_on_news_page and attempts <= MAX_SWIPES:
        attempts += 1
        context.perform_gesture('swipe_up', '')
        last_article_on_news_page = element_exists(context, 'div_will_bots')

    context.verify(['div_will_bots'])
