...
def run(context):
    we_build_text_link = False
    attempts = 0

    we_build_text_link = element_exists(context, 'lnk_we_build')
    
    while not we_build_text_link and attempts <= MAX_SWIPES:
        attempts += 1
        context.perform_gesture('swipe_up', '')
        we_build_text_link = element_exists(context, 'lnk_we_build')

    context.verify(['lnk_we_build'])
